...
public class SlettemeldingsFunksjoner {
	...
	public static DokumentInfo setAngreDokumentLogiskSlettet(Journalpost journalpost, DokumentInfo dokumentInfo) {
		Begrensning begrensning = dokumentInfo.getBegrensningnerByDokumentInfoIdOnly(dokumentInfo, BegrensningTypeCode.UTILGJENGELIGGJORT);
		if (begrensning == null) {
			begrensning = dokumentInfo.getBegrensningnerByDokumentInfoAndJournalpost(dokumentInfo, BegrensningTypeCode.UTILGJENGELIGGJORT, journalpost);
			if (begrensning == null) {
