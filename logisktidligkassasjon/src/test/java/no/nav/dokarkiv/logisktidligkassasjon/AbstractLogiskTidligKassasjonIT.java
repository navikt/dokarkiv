...
import no.nav.dokarkiv.logisktidligkassasjon.util.TestUtils;
...
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.RANDOM_PORT,
		classes = {CoreConfig.class, LogiskTidligKassasjonConfig.class, TestToolsAutoConfig.class})
...
@Transactional
public abstract class AbstractLogiskTidligKassasjonIT {
	...
	protected HttpHeaders createHeadersWithAksjon(String aksjon) throws IOException {
		...
		headers.add(HttpHeaders.AUTHORIZATION, oidcTokenPersonUserTest);
		headers.add(NAV_CONSUMER_TOKEN, oidcTokenServiceUserTest);
		headers.add(AksjonsLoggService.AKSJONS_INFO_HEADER, objectToJsonString(createAksjonsLoggRequest(TestUtils.journalpostId, TestUtils.dokumentInfoId, aksjon)));
		return headers;
	}
