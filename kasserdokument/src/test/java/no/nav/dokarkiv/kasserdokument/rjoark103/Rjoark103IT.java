...
public class Rjoark103IT extends AbstractKasserDokumentIT {
	...
	public void skalLagreAksjon() throws IOException {
		...
		ResponseEntity<String> responseEntity = restTemplate.exchange(
				URL_FYSISKTIDLIGKASSASJON,
				HttpMethod.DELETE,
				new HttpEntity<>(createKasserDokumentRequest(dokumentInfo1.getDokumentInfoId()), createHeadersWithAksjon(SLETT.name())),
				String.class);
	...
	public void skalIkkeLagreAksjonsLoggVedFeil() throws IOException {
		...
		ResponseEntity<String> responseEntity = restTemplate.exchange(
				URL_FYSISKTIDLIGKASSASJON,
				HttpMethod.DELETE,
				new HttpEntity<>(createKasserDokumentRequest(dokumentInfoId), createHeadersWithAksjon(SLETT.name())),
				String.class);
	...
	public void skallIkkeTidligtKassereDokument_ettersomDokmentInfoIkkeErLogiskKassert() throws IOException {
		...
		ResponseEntity<String> responseEntity = restTemplate.exchange(
				URL_FYSISKTIDLIGKASSASJON,
				HttpMethod.DELETE,
				new HttpEntity<>(createKasserDokumentRequest(dokumentInfo.getDokumentInfoId()), createHeadersWithAksjon(SLETT.name())),
				String.class);
	...
	public void skallTidligtKassereDokument_medDokmentKnyttetFlereJournalposter() throws IOException {
		...
		ResponseEntity<KasserDokumentResponse> responseEntity = restTemplate.exchange(
				URL_FYSISKTIDLIGKASSASJON,
				HttpMethod.DELETE,
				new HttpEntity<>(createKasserDokumentRequest(dokumentInfoRep.get().getDokumentInfoId()), createHeadersWithAksjon(SLETT.name())),
		...
		TestTransaction.start();
		assertThat(responseEntity.getStatusCode(), is(HttpStatus.OK));
		assertThat(responseEntity.getBody().getDokumentInfoId(), is(dokumentInfoRep.get().getDokumentInfoId()));
		dokumentInfoRep = dokumentinfoRepository.findByDokumentInfoId(dokumentInfo1.getDokumentInfoId());
		assertTrue(dokumentInfoRep.isPresent());
		assertThat(dokumentInfoRep.get().getKassertAvNavn(), is(KASSERT_AV_NAVN));
		assertNotNull(dokumentInfoRep.get().getDatoKassert());
	...
	public void noAccess() {
		...
		ResponseEntity<String> responseEntity = restTemplate.exchange(
				URL_FYSISKTIDLIGKASSASJON,
				HttpMethod.DELETE,
				new HttpEntity<>(createKasserDokumentRequest(dokumentInfo.getDokumentInfoId()), createNoAccessHeaders()),
				String.class);
