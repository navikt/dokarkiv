...
public class Rjoark103IT extends AbstractArkiverVariantIT {
	...
	public void shouldSaveFileAsSladdetVariant() {
		...
		FilDetaljer sladdetVariant = persistedDokumentInfo.findFilDetaljerByVariantFormat(SLADDET);
		assertThat(sladdetVariant).isNotNull();
		assertThat(sladdetVariant.getFiltype()).isEqualTo(PDF);
		assertThat(sladdetVariant.getFilstorrelse()).isEqualTo(Integer.toString(FIL.length));
		assertThat(sladdetVariant.getSha256Sjekksum()).containsExactly(sha256(FIL));
		DokumentFil dokumentFil = dokumentFilTestRepository.findByFilUuid(sladdetVariant.getFilUuid());
