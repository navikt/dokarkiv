...
public class GraphQlQueryIT {
	...
	public void shouldGetFil() throws Exception {
		...
		Journalpost otherJournalpost =  TestDataUtils.createJournalpostBuilder(FIL_UUID).build();
		otherJournalpost.findFilDetaljerByFilUuid(FIL_UUID).setVariantFormat(VariantFormatCode.BREVBESTILLING);
		joarkRepository.save(otherJournalpost);
