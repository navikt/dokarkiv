...
public class JournalfoerSkannetDokumentRestController {
	...
	public ResponseEntity<Void> bulkOppdaterLogiskVedlegg(@PathVariable String dokumentInfoId, @RequestBody List<String> titler) {
		RequestContextUtil.createAndSetUsername(MDC.get(MDC_USER_ID), MDC.get(MDC_CONSUMER_ID));
		log.info("bulkoppdaterlogiskvedlegg har mottatt kall. dokumentInfoId={}", dokumentInfoId);
        validateId(dokumentInfoId, DOKUMENT_INFO_ID_STRING);

		logiskVedleggService.bulkOppdaterLogiskVedlegg(dokumentInfoId, titler);
