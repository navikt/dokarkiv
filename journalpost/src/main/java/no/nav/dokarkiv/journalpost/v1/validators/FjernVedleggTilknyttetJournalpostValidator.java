...
public class FjernVedleggTilknyttetJournalpostValidator {
	...
	public void validateJournalpostDokumentInfoRelasjon(JournalpostDokumentInfoRelasjon journalpostDokumentInfoRelasjon) {
		if (!VEDLEGG.equals(journalpostDokumentInfoRelasjon.getTilknyttetJournalpostSom())) {
			throw new KanIkkeSlettetVedleggKnyttetTilJournalpostException("TilknytteJournalpost er ikke som vedlegg og kan ikke slettes ");
		}
	}
	...
	public void validateJournalPostStatusOgType(Journalpost utgaaendeJournalpost) {
		if (!U.equals(utgaaendeJournalpost.getJournalposttype()) &&
			...
			throw new KanIkkeSlettetVedleggKnyttetTilJournalpostException(String.format("Kan ikke slette vedlegg med journalpostId=%s, Journalpost må være utgående(U) og under arbeid(D)",
	...
	public void validateDokumentInfoOriginalJpNotEqualsInputJournalpost(DokumentInfo dokumentinfo, Long journalpostId) {
		...
		if (dokumentinfo == null && journalpostId==null) {
			throw new DokumentInfoIkkeFunnetException(String.format("Fant ikke source dokument og kan ikke slette vedlegg med journalpostId=%s",
					journalpostId));
		} else if (dokumentinfo.getOriginalJournalpost().getJournalpostId().equals(journalpostId)) {
			throw new KanIkkeSlettetVedleggKnyttetTilJournalpostException(String.format("JounalpostId er lik med originalJournalpostId og vedlagt kan ikke slettes. med journalpostId=%s",
					journalpostId));
		}
	}
