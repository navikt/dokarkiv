...
import static no.nav.freg.security.oidc.auth.idtoken.extract.NavHttpHeaders.NAV_CONSUMER_TOKEN_HEADER;
...
public class ValidateAdminConsumerAccessInterceptorTest {
	...
	private NavLdapService navLdapService = mock(NavLdapService.class);
	...
	private ValidateAdminConsumerAccessInterceptor validateAdminConsumerAccessInterceptor = new ValidateAdminConsumerAccessInterceptor(navLdapService);
	...
	public void shouldAllowAccessToJoarkadminServiceUserAndNavUserMemberOfJoarkVedlikeholdGroup() throws Exception {
		...
		request.addHeader(NAV_CONSUMER_TOKEN_HEADER, OIDC_TOKEN_JOARKADMIN_USER_TEST);
	...
	public void shouldDenyAccessToNonJoarkAdminConsumerUser() throws Exception {
		...
		MockHttpServletRequest request = new MockHttpServletRequest();
		request.addHeader(HttpHeaders.AUTHORIZATION, OIDC_TOKEN_ADMIN_USER);
		request.addHeader(NAV_CONSUMER_TOKEN_HEADER, OIDC_TOKEN_DOKARKIV_USER_TEST);
		MockHttpServletResponse response = new MockHttpServletResponse();
		boolean result = validateAdminConsumerAccessInterceptor.preHandle(request, response, new Object());
		assertThat(result, is(false));
