...
public class ValidateAdminConsumerAccessInterceptorTest {
	...
	public void shouldAllowAccessToJoarkadminServiceUserAndNavUserMemberOfJoarkVedlikeholdGroup() throws Exception {
		when(azureAdGraphService.userInGroup("Z990067", ADGRUPPE_JOARK_VEDLIKEHOLD)).thenReturn(true);
		MockHttpServletRequest request = new MockHttpServletRequest();
		request.addHeader(AUTHORIZATION, OIDC_TOKEN_ADMIN_USER);
		request.addHeader(NAV_CONSUMER_TOKEN, OIDC_TOKEN_JOARKADMIN_USER_TEST);
	...
	public void shouldDenyAccessToNonJoarkAdminConsumerUserWhenOnlyAuthorizationTokenIsPresent() throws Exception {
		when(azureAdGraphService.userInGroup("Z990782", ADGRUPPE_JOARK_VEDLIKEHOLD)).thenReturn(true);
		MockHttpServletRequest request = new MockHttpServletRequest();
		request.addHeader(AUTHORIZATION, OIDC_TOKEN_DOKARKIV_USER_TEST);
		MockHttpServletResponse response = new MockHttpServletResponse();

		boolean result = validateAdminConsumerAccessInterceptor.preHandle(request, response, new Object());

		assertThat(result).isFalse();
		assertThat(response.getErrorMessage()).isEqualTo("OIDC token på Authorization-header må tilhøre servicebruker på srvjoarkadmin");
	}
	...
	public void shouldDenyAccessToNonJoarkAdminConsumerUser() throws Exception {
		...
		MockHttpServletRequest request = new MockHttpServletRequest();
		request.addHeader(AUTHORIZATION, OIDC_TOKEN_ADMIN_USER);
		request.addHeader(NAV_CONSUMER_TOKEN, OIDC_TOKEN_DOKARKIV_USER_TEST);
		MockHttpServletResponse response = new MockHttpServletResponse();

		boolean result = validateAdminConsumerAccessInterceptor.preHandle(request, response, new Object());

		assertThat(result).isFalse();
		assertThat(response.getErrorMessage()).isEqualTo("OIDC token på Nav-Consumer-Token header må tilhøre serviceuser på srvjoarkadmin");
	}

	@Test
	public void shouldDenyAccessToNavUserNotMemberOfJoarkVedlikehold() throws Exception {
		when(azureAdGraphService.userInGroup("Z990782", ADGRUPPE_JOARK_VEDLIKEHOLD)).thenReturn(false);
		MockHttpServletRequest request = new MockHttpServletRequest();
		request.addHeader(AUTHORIZATION, OIDC_TOKEN_ADMIN_USER);
		request.addHeader(NAV_CONSUMER_TOKEN, OIDC_TOKEN_JOARKADMIN_USER_TEST);
		MockHttpServletResponse response = new MockHttpServletResponse();
		...
		assertThat(result).isFalse();
		assertThat(response.getErrorMessage()).isEqualTo("NAVIdent må være medlem av gruppen guid=\"0000-GA-joark-vedlikehold\" i Azure AD");
	}

	@Test
	public void shouldAllowAccessWhenAzureTokenAndWorkaround() throws Exception {
		MockHttpServletRequest request = new MockHttpServletRequest();
		request.addHeader(AUTHORIZATION, OIDC_TOKEN_AZURE);
		MockHttpServletResponse response = new MockHttpServletResponse();
