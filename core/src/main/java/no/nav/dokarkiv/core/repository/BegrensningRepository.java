...
public interface BegrensningRepository extends CrudRepository<Begrensning, Long> {
    ...
    @Query(value = "SELECT * FROM T_BEGRENSNING beg WHERE not beg.DOKUMENT_INFO_ID=:dokumentInfoId and beg.journalpost_id = :journalpostId and beg.begrensning_type = :begrensningType", nativeQuery = true)
