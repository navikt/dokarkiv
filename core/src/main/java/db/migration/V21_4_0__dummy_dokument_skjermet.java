...
public class V21_4_0__dummy_dokument_skjermet implements JdbcMigration {
	...
	private void addOrUpdateDummyDocument(Connection connection, String query) throws Exception {
		try (InputStream in = new ClassPathResource("dummy_dokument_skjermet.pdf").getInputStream();
			 PreparedStatement pstmt = connection.prepareStatement(query)) {
			pstmt.setBlob(1, in);
			pstmt.setString(2, FIL_UUID_DUMMY_DOKUMENT_SKJERMET);
			pstmt.execute();
			connection.commit();
		}
	}
	...
	private boolean isDummyDocumentExists(Connection connection) throws Exception {
		try (PreparedStatement existingFile = connection.prepareStatement("select 'exists' from T_DOKUMENT_FIL where FIL_UUID=?")) {
			existingFile.setString(1, FIL_UUID_DUMMY_DOKUMENT_SKJERMET);
