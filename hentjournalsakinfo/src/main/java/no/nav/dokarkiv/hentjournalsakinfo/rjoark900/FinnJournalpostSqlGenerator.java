...
final class FinnJournalpostSqlGenerator {
	...
	static String finnJournalposterSql(JournalpostFilter journalpostFilter, List<String> cteAliases, String gsakCte, boolean brukParallelHint) {
		return "WITH psaksaker AS\n" +
			   ...
			   "     )\n" +
			   "SELECT " + parallelHint(brukParallelHint) + " r.*,\n" +
			   ...
			   "                SELECT *\n" +
			   "                FROM (\n" +
			   "                       SELECT " + parallelHint(brukParallelHint) + " j.journalpost_id,\n" +
	...
	private static String parallelHint(boolean brukParallelHint) {
		return brukParallelHint ? "/*+ PARALLEL(8) */" : "";
	}
